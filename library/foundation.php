	...
	function thorpress_pagination() {
		...
		$paginate_links = paginate_links(
			array(
				...
				'mid_size'  => 5,
				'prev_next' => true,
				'prev_text' => __( '&laquo;', 'thorpress' ),
				'next_text' => __( '&raquo;', 'thorpress' ),
				'type'      => 'list',
			)
		);
...
if ( ! function_exists( 'thorpress_menu_fallback' ) ) :
	function thorpress_menu_fallback() {
		echo '<div class="alert-box secondary">';
		/* translators: %1$s: link to menus, %2$s: link to customize. */
		printf(
			__( 'Please assign a menu to the primary menu location under %1$s or %2$s the design.', 'thorpress' ),
			/* translators: %s: menu url */
			sprintf(
				__( '<a href="%s">Menus</a>', 'thorpress' ),
				get_admin_url( get_current_blog_id(), 'nav-menus.php' )
			),
			/* translators: %s: customize url */
			sprintf(
				__( '<a href="%s">Customize</a>', 'thorpress' ),
...
if ( ! function_exists( 'thorpress_active_nav_class' ) ) :
	function thorpress_active_nav_class( $classes, $item ) {
		if ( $item->current == 1 || $item->current_item_ancestor == true ) {
			$classes[] = 'is-active';
		}
		return $classes;
	}
	add_filter( 'nav_menu_css_class', 'thorpress_active_nav_class', 10, 2 );
endif;
 ...
 */
if ( ! function_exists( 'thorpress_active_list_pages_class' ) ) :
	function thorpress_active_list_pages_class( $input ) {
	...
	add_filter( 'wp_list_pages', 'thorpress_active_list_pages_class', 10, 2 );
endif;
...
if ( ! function_exists( 'thorpress_responsive_video_oembed_html' ) ) :
	function thorpress_responsive_video_oembed_html( $html, $url, $attr, $post_id ) {
	...
	add_filter( 'embed_oembed_html', 'thorpress_responsive_video_oembed_html', 10, 4 );
endif;
...
if ( ! function_exists( 'thorpress_mobile_menu_id' ) ) :
	function thorpress_mobile_menu_id() {
...
if ( ! function_exists( 'thorpress_title_bar_responsive_toggle' ) ) :
	function thorpress_title_bar_responsive_toggle() {
